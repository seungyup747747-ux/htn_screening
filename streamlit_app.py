"""
고혈압 위험 스크리닝 웹앱 (Streamlit) — v2
- 모델: SMOTE Logistic Regression (Top17) — KNHANES 9기 (2022, 2024) 학습
- 임계값: OOF 민감도 85% 목표 기준 데이터 기반 도출 (≈0.427)
- 입력: 비침습 자가보고 변수 17개 (혈압계 없이)
- 출력: 스크리닝 위험점수 + 데이터 기반 임계값 기준 분류
- 데이터 인코딩: KNHANES 9기 원본 코드 유지 (PyCaret 자동 전처리)
"""
import json
import os
import pickle
...
try:
    ...
    # v2 모델 기준 기본값 (JSON 누락 시 fallback)
    screening_config = {
        "screening_threshold": 0.427,
        "target_sensitivity": 0.85,
        "model": "LogisticRegression",
        "strategy": "SMOTE",
        "feature_version": "Top17",
        "score_method": "predict_proba",
        "input_variables": [
            "age", "dyslipidemia_dx", "bmi", "fh_htn_sibling",
            "drink_amount", "fh_htn_father", "self_health", "fh_htn_mother",
            "waist", "edu", "weight_change", "body_shape",
            "occupation", "sex", "diabetes_dx", "income", "sugar_g",
        ],
    }

SCREENING_THRESHOLD = float(screening_config.get("screening_threshold", 0.427))
...
st.markdown(
    "**KNHANES 9기 (2022, 2024) 데이터 기반 머신러닝 모델 v2** — "
    "혈압계 없이 17개 설문 응답만으로 고혈압 위험 수준을 1차 점검합니다."
)
st.info(
...
st.sidebar.header("📋 설문 응답 (17문항)")
st.sidebar.caption(
    f"전략: {screening_config.get('strategy', 'SMOTE')} • "
    f"모델: {screening_config.get('model', 'LogisticRegression')} • "
    f"변수: {screening_config.get('feature_version', 'Top17')} • "
...
def yes_no_unknown(label, key, help_text=None):
    """0=아니오, 1=예, np.nan=모름 (KNHANES: 8=모름 → 결측 처리)"""
    choice = st.selectbox(
        label,
        options=["아니오", "예", "모름"],
        index=0,
...
# ----------------------------------------------------------------------
# 👤 기본 정보 (5개): age, sex, bmi, waist, edu
# ----------------------------------------------------------------------
with st.sidebar.expander("👤 기본 정보", expanded=True):
    ...
    input_data["sex"] = st.selectbox(
        "성별",
        options=[1, 2],
        format_func=lambda x: "1. 남성" if x == 1 else "2. 여성",
    )
...
# ----------------------------------------------------------------------
# 💰 사회경제 정보 (2개): income, occupation
# ----------------------------------------------------------------------
with st.sidebar.expander("💰 사회경제 정보", expanded=True):
...
# ----------------------------------------------------------------------
# 🍷 음주·식이 (2개): drink_amount, sugar_g
# ----------------------------------------------------------------------
with st.sidebar.expander("🍷 음주·식이", expanded=True):
    ...
    input_data["sugar_g"] = st.number_input(
        "당류 일일 섭취량 (g/일)",
        0.0, 500.0, 60.0, step=1.0,
        help="24시간 회상 기준 일평균 당류 섭취량 (N_SUGAR)",
    )

# ----------------------------------------------------------------------
# 🪞 건강 인식·체형 (3개): self_health, body_shape, weight_change
# ----------------------------------------------------------------------
with st.sidebar.expander("🪞 건강 인식·체형", expanded=True):
    ...
    input_data["body_shape"] = st.selectbox(
        "본인이 인지하는 체형",
        options=[1, 2, 3, 4, 5],
        format_func=lambda x: {
            1: "1. 매우 마름",
    ...
    input_data["weight_change"] = st.selectbox(
        "최근 1년 체중 변화",
        options=[1, 2, 3],
        format_func=lambda x: {
            1: "1. 변화 없음",
...
# ----------------------------------------------------------------------
# 🩺 기저질환 (2개): dyslipidemia_dx, diabetes_dx
# ----------------------------------------------------------------------
with st.sidebar.expander("🩺 기저질환", expanded=True):
    ...
    input_data["diabetes_dx"] = yes_no_unknown(
        "당뇨병 의사진단 받은 적 있음",
        key="diabetes_dx",
        help_text="당뇨병으로 진단받은 경우",
    )

# ----------------------------------------------------------------------
# 👨‍👩‍👧 가족력 (3개): fh_htn_father, fh_htn_mother, fh_htn_sibling
# ----------------------------------------------------------------------
...
if predict_btn:
    ...
    if is_positive:
        st.error(
            ...
            "※ 본 도구의 양성 판정 정확도(특이도)는 약 70%이므로, "
        ...
        st.success(
            ...
            "※ 본 도구의 음성 판정 정확도(NPV)는 약 90%로, "
            "약 10%의 음성 판정은 실제 고혈압을 놓칠 수 있습니다."
        )
...
st.caption(
    ...
    f"**Model**: {screening_config.get('strategy', 'SMOTE')} "
    f"{screening_config.get('model', 'LogisticRegression')} "
    f"({screening_config.get('feature_version', 'Top17')})  •  "
    "**Holdout Sensitivity**: 86.8%  •  **Specificity**: 69.6%  •  "
    "**NPV**: 90.4%  •  **ROC-AUC**: 0.866"
)
st.caption(

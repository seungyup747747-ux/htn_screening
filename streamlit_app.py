...
with st.sidebar.expander("🚬 생활습관"):
    ...
    input_data["drink_freq"] = st.slider(
        "음주 빈도 (0=안 마심, 1~7 등급)", 0, 7, 1,
        help="0: 평생 안 마심  •  1~7: KNHANES 음주 빈도 등급"
    )
    input_data["drink_amount"] = st.slider(
        "음주량 (0=안 마심, 1~7 등급)", 0, 7, 1
    )
...
with st.sidebar.expander("🧠 정신건강"):
    ...
    input_data["phq9"] = st.number_input(
        ...
        help="0~4 정상  •  5~9 경증  •  10~14 중등도  •  15+ 중증"
    )
...
def predict_htn(model, input_dict, required_cols):
    ...
    # 안전장치 — 타깃 컬럼이 어쩌다 들어와 있으면 제거
    if TARGET_COL in df.columns:
        df = df.drop(columns=[TARGET_COL])

    # sklearn 모델·파이프라인 직접 호출
    y_pred = model.predict(df)
    label = int(y_pred[0])

    # 확률 계산 — predict_proba 우선, 없으면 decision_function + sigmoid
    proba = None
...
if predict_btn:
    with st.spinner("예측 중..."):
        try:
            proba, label, pred_full = predict_htn(model, input_data, required_features)
        except Exception as e:
